...
/*
    ...
    基础命令和工具
        ...
        6.ulimit用户资源
            Linux系统对每个登录的用户都限制其最大进程和打开的最大文件句柄数。为了提高性能，
            可以根据硬件资源的具体情况设置各个用户的最大进程数和打开的最大文件句柄。可以用
            ulimit -a来显示当前的各种系统对用户使用资源的限制：
        ...
        7.curl http
            由于当前的线上服务较多地使用了RESTful风格的API，所以集成测试就需要进行HTTP调用，
